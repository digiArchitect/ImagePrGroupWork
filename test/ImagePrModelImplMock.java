...
/**
 * Mocks the image processor to test script files.
 */
public class ImagePrModelImplMock implements ImagePrModel {
   ...
   */
  public ImagePrModelImplMock(StringBuilder log) {
    this.log = log;
  ...
  public HashMap<Integer, List<Integer>> histogram(String fileLoc) {
    ...
    for (int x = 0; x < 257; x++) {
      ...
      for (String s : components) {
        ...
        int count = pixelVals.stream().filter(y -> y == finalX).mapToInt(y -> y).sum();
